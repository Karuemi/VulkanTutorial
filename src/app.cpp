...
void HelloTriangleApplication::pickPhysicalDevice() {
    ...
    for (const auto& dev : devices) {
        if (isDeviceSuitable(dev)) {
            ...
            printf("MSAA: found maximum %i samples\n", static_cast<uint32_t>(msaaSamples));
            break;
        }
    }
...
void HelloTriangleApplication::createSwapChain() {
    ...
    createInfo.preTransform = details.capabilities.currentTransform; // no support for transformation

    createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR; // ignore alpha channel window-system wise

    createInfo.presentMode = presentMode;
    createInfo.clipped = VK_TRUE;

    createInfo.oldSwapchain = VK_NULL_HANDLE; 
    // we dont want to resize the window as it might break
    // we only create one swapchain per app

    if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapchain) != VK_SUCCESS) {
        throw std::runtime_error("failed to create swapchain");
    }

    printf("SwapChain: created successfuly\n");
...
VkSurfaceFormatKHR HelloTriangleApplication::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    for (const auto& availableFormat : availableFormats) {
        if (availableFormat.format == VK_FORMAT_B8G8R8A8_SRGB && availableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
            printf("SwapChain: picked optimized format\n");
...
VkPresentModeKHR HelloTriangleApplication::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availableModes) {
    for (const auto& availableMode : availableModes) {
        if (availableMode == VK_PRESENT_MODE_MAILBOX_KHR) {
            printf("SwapChain: enabled triple buffering\n");
            return availableMode;
        }
        if (availableMode == VK_PRESENT_MODE_FIFO_LATEST_READY_KHR) {
            printf("SwapChain: enabled FIFO_LATEST_READY\n");
            return availableMode;
        }
    }
...
VkExtent2D HelloTriangleApplication::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        printf("SwapChain: set dimensions as %d %d\n", capabilities.currentExtent.width, capabilities.currentExtent.height);
...
void HelloTriangleApplication::createRenderPass() {
    /// COLOR ATTACHMENT
    // what should happen with color before rendering
    ...
    VkSubpassDependency dependency{}; // something to do with semaphores and waiting
...
void HelloTriangleApplication::createGraphicsPipeline() {
    ...
    /// VIEWPORT 
    // where to render stuff on framebuffer
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    ...
    /// SCISSOR
    // cuts the image to put into viewport
    VkRect2D scissor{};
    scissor.offset = {0, 0};
    scissor.extent = swapChainExtent;

    /// DYNAMIC STATES
    //makes viewport resizeable and stuff, very convinient
    ...
    // Static coded - requires new pipelines to change that
    // for dynamic just delete assignment to structures
    ...
    // deriving from existing pipeline - not used here
...
void HelloTriangleApplication::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    // since its dynamic to use viewport and scissors
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
...
void HelloTriangleApplication::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
    ...
    vkQueueWaitIdle(graphicsQueue); // might optimize here with fences but not needed for one transfer

    vkFreeCommandBuffers(device, commandPool, 1, &commandBuffer);
}

VkCommandBuffer HelloTriangleApplication::beginSetupCommands() {
    return beginSingleTimeCommands();
}

void HelloTriangleApplication::flushSetupCommands(VkCommandBuffer commandBuffer) {
    vkEndCommandBuffer(commandBuffer);
...
void HelloTriangleApplication::createTextureImage() {
    ...
    VkCommandBuffer setupCommandBuffer = beginSetupCommands();
    ...
    transitionImageLayout(setupCommandBuffer, textureImage, VK_FORMAT_R8G8B8A8_SRGB, 
        VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);

    copyBufferToImage(setupCommandBuffer, stagingBuffer, textureImage, 
        static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));

    // no need for this because of mipmaps
    // transitionImageLayout(setupCommandBuffer, textureImage, VK_FORMAT_R8G8B8A8_SRGB, 
    //     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels);

    flushSetupCommands(setupCommandBuffer);

    generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels); // might integrate command flushing here
...
void HelloTriangleApplication::transitionImageLayout(VkCommandBuffer commandBuffer, VkImage image, VkFormat format, 
    ...
    // flags so that validation layers won't complain
...
void HelloTriangleApplication::copyBufferToImage(VkCommandBuffer commandBuffer, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
...
void HelloTriangleApplication::createTextureSampler() {
    ...
    samplerInfo.magFilter = VK_FILTER_LINEAR; // concerns oversampling
    samplerInfo.minFilter = VK_FILTER_LINEAR; // concerns undersampling
    ...
    samplerInfo.maxLod = VK_LOD_CLAMP_NONE; // means all of mipmaps will be used
...
void HelloTriangleApplication::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
    // check for support for linear blitting
...
void HelloTriangleApplication::createSyncObjects() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
            ...
            throw std::runtime_error("failed to create sync objects for frame (semaphores, fences)");
        }
    }
}
...
void HelloTriangleApplication::createLogicalDevice() {
    ...
    deviceFeatures.sampleRateShading = VK_TRUE; // enable sample shading feature for the device - performance cost

    VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR fifoLatestReadyFeature{}; // should only work on nvidia rtx 4050 hence this
    fifoLatestReadyFeature.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PRESENT_MODE_FIFO_LATEST_READY_FEATURES_KHR;
    fifoLatestReadyFeature.presentModeFifoLatestReady = VK_TRUE;

    VkDeviceCreateInfo createInfo{};
    createInfo.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
    createInfo.pNext = &fifoLatestReadyFeature; // added for rtx4050
...
void HelloTriangleApplication::drawFrame() {
    ...
    // Only reset if we submit work
    vkResetFences(device, 1, &inFlightFences[currentFrame]);

    updateUniformBuffer(currentFrame);

    // Record and submit command buffer to the graphics queue
...
void HelloTriangleApplication::loadModel() {
    ...
    printf("Model: has %zu vertices that are shared in %zu ways\n", vertices.size(), indices.size());
}
